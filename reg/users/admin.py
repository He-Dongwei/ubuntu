...
from .models import User ,bill,accessrecord,payment,company,comment
...
class UserAdmin(BaseUserAdmin):
    fieldsets = (
        (_('必须信息'), {
            'fields': ('email', 'password','realname','sex','phonenumber','job','workunit',)
        }),
        (_('选填信息'),{
            'fields':('email2','officephone','address','postcode')
        }),
        (_('Permissions'), {
    ...
    list_display = ('email', 'sex','is_active',)
...
class accessrecordAdmin(admin.ModelAdmin):
    list_display=('user','time','page','IP','comment')  
